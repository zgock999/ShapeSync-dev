...
### Q2. 後工程の Generate 実行時にトポロジ不一致エラー（`FBM topology does not match Base`）が発生する
* **原因**:
  Base と FBM の間でメッシュの頂点構造（トポロジ）が異なっています（第3章の Generate 時に `FigureGenerateMeshBuildFailed: FigureMeshBuildInvalid: FBM topology does not match Base: SampleI` などの診断が表示されます）。微小なアクセサリーや衣装パーツの消し残し、またはポリゴン削減設定の違いなどによって頂点構成がずれている可能性があります。
* **解決策**:
