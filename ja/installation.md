...
> [!WARNING]
> URL 内の `?path=Packages/net.zgock-lab.shapesync` は必ず `#0.2.0-preview13` より前に記述してください。順序が異なると Git の取得エラー（pathspec error）が発生します。
...
* **解決策**: `?path=Packages/net.zgock-lab.shapesync` を `#0.2.0-preview13` の前に記述しているか確認してください。
