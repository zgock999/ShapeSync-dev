...
namespace zgock.ShapeSync.Tests.PlayMode
{
    public sealed class TextureStackMachineRuntimeTests
    {
        ...
        public IEnumerator Fill_CompletesAndTransfersOneExactEdgeDelivery()
        {
            ...
            Assert.That(executor.TryExecute(new TextureRecipeStub(document, new[] { new TextureBindingEntry { logicalName = "out", kind = TextureBindingKind.OutputHall } }), new TextureExecutionOriginKey(1), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
        ...
        public IEnumerator TryExecute_PropagatesHostHallReservationDiagnostic()
        {
            ...
            try
            {
                ...
                try
                {
                    Assert.That(new TextureExecutor(host).TryExecute(FillStub(Color.red), new TextureExecutionOriginKey(987), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.False);
        ...
        {
#if UNITY_EDITOR
            yield return AssertGpuPixel("0.125 0.25 0.75 0.5 FILL .", new[] { Output() }, new Vector4(0.125f, 0.25f, 0.75f, 0.5f), 31);
#else
            Assert.Ignore("Compute asset loading is Editor-only.");
            yield break;
#endif
        ...
        public IEnumerator IngestAndPublish_TransferExactSourceTexelThroughGpu()
        {
            ...
            Assert.That(new TextureExecutor(host).TryExecute(stub, new TextureExecutionOriginKey(32), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
            while (!handle.IsCompleted) yield return null;
            Assert.That(handle.Succeeded, Is.True, handle.Diagnostic?.message);
            Assert.That(handle.Result.TryTakeDelivery(out TextureDelivery delivery), Is.True);
        ...
        public IEnumerator NativeRectangleCanvas_DeliversRenderTexture()
        {
            ...
            Assert.That(new TextureExecutor(host).TryExecute(stub, new TextureExecutionOriginKey(2), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
            while (!handle.IsCompleted) yield return null;
            Assert.That(handle.Succeeded, Is.True, handle.Diagnostic?.message);
            Assert.That(handle.Result.TryTakeDelivery(out TextureDelivery delivery), Is.True);
        ...
        public IEnumerator NativeRectangleAlphaComposite_DeliversRenderTexturePixels()
        {
            ...
            Assert.That(new TextureExecutor(host).TryExecute(stub, new TextureExecutionOriginKey(25), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
            while (!handle.IsCompleted) yield return null;
            Assert.That(handle.Succeeded, Is.True, handle.Diagnostic?.message);
            Assert.That(handle.Result.TryTakeDelivery(out TextureDelivery delivery), Is.True);
            Assert.That(delivery.Texture, Is.TypeOf<RenderTexture>());
        ...
        public IEnumerator Yuv_UsesDisplacedIndependentInputHalls()
        {
            ...
            Assert.That(new TextureExecutor(host).TryExecute(stub, new TextureExecutionOriginKey(3), out TextureExecutionHandle handle, out StackMachineDiagnostic d), Is.True, d?.message);
        ...
        public IEnumerator Colorize_ReplacesChromaPreservesAlphaAndUsesGpu()
        {
            ...
            yield return AssertGpuPixel("$source 0.6666667 1 0 COLORIZE .", new[] { Source("source", source), Output() }, new Vector4(0.4278f, 0.4278f, 1f, 0.25f), 32, 0.005f);
            Object.Destroy(source);
#else
            Assert.Ignore("Compute asset loading is Editor-only."); yield break;
#endif
        ...
        public IEnumerator Colorize_UnitHueRoundTripsAtFullSaturation()
        {
            ...
            })
            {
                ...
                yield return AssertGpuPixel("$source " + sample.hue.ToString(System.Globalization.CultureInfo.InvariantCulture) + " 1 0 COLORIZE .", new[] { Source("source", source), Output() }, sample.expected, 32, 0.005f);
                Object.Destroy(source);
            }
#else
            Assert.Ignore("Compute asset loading is Editor-only."); yield break;
        ...
        public IEnumerator AlphaCopy_UsesStraightSourceOverOnGpu()
        {
            ...
            yield return AssertGpuPixel("$bottom $top ACOPY .", new[] { Source("bottom", bottom), Source("top", top), Output() }, new Vector4(0.6f, 0f, 0.4f, 0.625f), 11);
        ...
        public IEnumerator PremultipliedAlphaCopy_UsesPremultipliedSourceOverOnGpu()
        {
            ...
            yield return AssertGpuPixel("$bottom $top PACOPY .", new[] { Source("bottom", bottom), Source("top", top), Output() }, new Vector4(0.375f, 0f, 0.25f, 0.625f), 12);
            Object.Destroy(bottom); Object.Destroy(top);
#else
            Assert.Ignore("Compute asset loading is Editor-only."); yield break;
#endif
        ...
        public IEnumerator Alpha_ReplacesSourceAlphaWithGrayRgbAverageOnGpu()
        {
            ...
            yield return AssertGpuPixel("$source $gray ALPHA .", new[] { Source("source", source), Source("gray", gray), Output() }, new Vector4(0.2f, 0.4f, 0.6f, 0.6f), 19);
        ...
        public IEnumerator StackBuiltins_PreserveDuplicatedIntermediateBranchesOnGpu()
        {
            ...
            yield return AssertGpuPixel("$iris $a ACOPY DUP $b ACOPY SWAP $c ACOPY ADD .", new[] { Source("iris", iris), Source("a", a), Source("b", b), Source("c", c), Output() }, new Vector4(1f, 0f, 1f, 2f), 18);
        ...
        public IEnumerator CopyAddMultiplyAndNormalWeightedBlend_ProduceExpectedGpuPixels()
        {
            ...
            yield return AssertGpuPixel("$a $out COPY DROP", new[] { Source("a", a), Output() }, new Vector4(0.2f, 0.4f, 0.6f, 0.8f), 13);
            yield return AssertGpuPixel("$a $b ADD $out COPY DROP", new[] { Source("a", a), Source("b", b), Output() }, new Vector4(0.7f, 0.65f, 1.35f, 1.3f), 14);
            yield return AssertGpuPixel("1 1 1 1 FILL $a SUB .", new[] { Source("a", a), Output() }, new Vector4(0.8f, 0.6f, 0.4f, 0.2f), 20);
            yield return AssertGpuPixel("$a $b MULTIPLY $out COPY DROP", new[] { Source("a", a), Source("b", b), Output() }, new Vector4(0.1f, 0.1f, 0.45f, 0.4f), 15);
            var baseNormal = Solid(new Color(0.5f, 0.5f, 1f, 1f));
            var detailNormal = Solid(new Color(1f, 0.5f, 0.5f, 1f));
            yield return AssertGpuPixel("$base $detail 0.25 NORMAL_WEIGHTED_BLEND $out COPY DROP", new[] { Source("base", baseNormal), Source("detail", detailNormal), Output() }, new Vector4(0.658114f, 0.5f, 0.974342f, 1f), 16, 0.005f);
        ...
        public IEnumerator NormalVectorWords_AccumulateDeltaAndNormalizeOnlyAtFinalize()
        {
            ...
            Assert.That(new TextureExecutor(host).TryExecute(stub, new TextureExecutionOriginKey(30), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
            while (!handle.IsCompleted) yield return null;
            Assert.That(handle.Succeeded, Is.True, handle.Diagnostic?.message);
            Assert.That(handle.Result.TryTakeDelivery(out TextureDelivery delivery), Is.True);
        ...
        public IEnumerator Resample_UsesPixelCenterBilinearSamplingAndClampOnGpu()
        {
            ...
            var stub = new TextureRecipeStub(document, new[] { Source("source", source), Output() });
            Assert.That(new TextureExecutor(host).TryExecute(stub, new TextureExecutionOriginKey(17), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
            while (!handle.IsCompleted) yield return null;
            Assert.That(handle.Succeeded, Is.True, handle.Diagnostic?.message);
            Assert.That(handle.Result.TryTakeDelivery(out TextureDelivery delivery), Is.True);
        ...
        public IEnumerator SrgbSquareSource_IngestsAsLinearGpuColor()
        {
            ...
            yield return AssertGpuPixel("$source .", new[] { Source("source", source), Output() }, new Vector4(linear, linear, linear, 1f), 26, 0.002f);
            Object.Destroy(source);
#else
            Assert.Ignore("Compute asset loading is Editor-only."); yield break;
#endif
        ...
        public IEnumerator Queue_CoalescesPendingCancelsConsumerAndMarksSubmittedWorkStale()
        {
            ...
            var root = CreateHost(out TextureStackMachineHost host);
            var executor = new TextureExecutor(host);
            Assert.That(executor.TryExecute(FillStub(Color.red), new TextureExecutionOriginKey(21), out TextureExecutionHandle coalesced, out StackMachineDiagnostic coalescedDiagnostic), Is.True, coalescedDiagnostic?.message);
            Assert.That(executor.TryExecute(FillStub(Color.green), new TextureExecutionOriginKey(21), out TextureExecutionHandle replacement, out StackMachineDiagnostic replacementDiagnostic), Is.True, replacementDiagnostic?.message);
            ...
            Assert.That(executor.TryExecute(FillStub(Color.blue), new TextureExecutionOriginKey(22), out TextureExecutionHandle cancelled, out StackMachineDiagnostic cancelledDiagnostic), Is.True, cancelledDiagnostic?.message);
            cancelled.Dispose();
            ...
            Assert.That(executor.TryExecute(FillStub(Color.white), new TextureExecutionOriginKey(21), out TextureExecutionHandle newest, out StackMachineDiagnostic newestDiagnostic), Is.True, newestDiagnostic?.message);
...
#if UNITY_EDITOR
            var root = CreateHost(out TextureStackMachineHost host);
            Assert.That(new TextureExecutor(host).TryExecute(FillStub(Color.cyan), new TextureExecutionOriginKey(24), out TextureExecutionHandle handle, out StackMachineDiagnostic diagnostic), Is.True, diagnostic?.message);
            yield return null;
            Assert.That(host.HasSubmittedRequest, Is.True);
            handle.Dispose();
        ...
        private static IEnumerator AssertGpuPixel(string wordSource, TextureBindingEntry[] bindings, Vector4 expected, ulong origin, float tolerance = 0.002f)
        {
            var root = CreateHost(out TextureStackMachineHost host);
